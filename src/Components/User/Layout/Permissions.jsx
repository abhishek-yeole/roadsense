...
const Permissions = () => {
    ...
    const showSpeed = (position) => {
      if (position.coords.speed !== null) {
        setSpeed(position.coords.speed.toFixed(2) * 3.6 + ' m/s');
      } else {
        ...
        <div>
			...
			<Accordion  expanded={expanded === 'panel4'} onChange={handleChange('panel4')}>
                <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls="panel4a-content" id="panel4a-header" >
                    <Typography fontSize={'14px'}><Icon icon="mdi:location" /> Speed {error ? (<Icon icon="ep:circle-check-filled" color="green" />) : (<Icon icon="mdi:denied" color="red" />)}</Typography>
                </AccordionSummary>
                <AccordionDetails>
                    <Typography fontSize={'small'} textAlign={'justify'}>
						<div>
